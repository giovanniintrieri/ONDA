...
export default function Home() {
 ...
 async function importFiles(files: File[], demo = false) {
  ...
  function beginQueue(track: Track | undefined, ids: string[], random = shuffle) {
    ...
    if (!first) return;
    void playerCommand('setQueue', { ids: order, startId: first.id, original: [...new Set(ids)], shuffle: random });
  }
  ...
  function showMediaDiagnostics() {
    window.alert(`Onda Android · Player nativo\nBrani in coda: ${queue.length}\nStato: ${playing ? 'in riproduzione' : 'in pausa'}\n${nativePlayer.error || 'Nessun errore'}`);
  }
