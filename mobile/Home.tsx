...
import { usePlayer, command } from './player';
import { useCallback, useEffect, useMemo, useRef, useState, type CSSProperties, type ReactNode } from 'react';
import { Download, ArrowRight, AudioLines, Check, ChevronRight, Clock3, FolderHeart, FolderOpen, HardDrive, Heart, Headphones, Library, ListMusic, Loader2, MoreHorizontal, Pause, Play, Plus, Repeat, Repeat1, Search, Shuffle, SkipBack, SkipForward, SlidersHorizontal, Sparkles, Trash2, Volume2, VolumeX, X } from 'lucide-react';
...
import { DEFAULT_SETTINGS, randomSeed, selectTracks, validateAlgorithm, type ListeningSettings, type Selection } from '@/lib/algorithm';
...
import { playlistGenres, playlistTrackIds } from './playlist-genres';
import { AUTO_PLAYLISTS, automaticTracks } from './library-insights';
import { DiagnosticsDialog, DuplicatesDialog } from './LibraryTools';
...
function Navigation({ view, go, playlists, counts, onCreate, bytes, onInfo, onDuplicates, onDiagnostics }: { view: View; go: (v: View) => void; playlists: Playlist[]; counts: number[]; onCreate: () => void; bytes: number; onInfo: () => void; onDuplicates: () => void; onDiagnostics: () => void }) {
  ...
  return <Sidebar className="app-sidebar"><SidebarHeader className="brand-area"><Brand /><span className="local-label">LOCAL MUSIC PLAYER</span></SidebarHeader><SidebarContent>
    <SidebarMenu className="primary-nav">
      {([{ id: 'home', label: 'Per te', icon: Sparkles }, { id: 'library', label: 'La tua libreria', icon: Library, count: counts[0] }, { id: 'favorites', label: 'Preferiti', icon: Heart, count: counts[1] }, { id: 'algorithm', label: 'Il tuo algoritmo', icon: SlidersHorizontal }, { id: 'search', label: 'Cerca musica', icon: Search }, { id: 'downloads', label: 'Scarica musica', icon: Download }] as const).map(item => <SidebarMenuItem key={item.id}><SidebarMenuButton isActive={view === item.id} onClick={() => nav(item.id)} className="nav-item"><item.icon /><span>{item.label}</span>{'count' in item && <small>{item.count}</small>}</SidebarMenuButton></SidebarMenuItem>)}
    </SidebarMenu>
    <div className="playlist-heading"><span>PLAYLIST AUTOMATICHE</span></div>
    <SidebarMenu className="playlist-nav">{AUTO_PLAYLISTS.map(p => <SidebarMenuItem key={p.id}><SidebarMenuButton className="nav-item" isActive={view === p.id} onClick={() => nav(p.id)}><ListMusic /><span>{p.name}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu>
    ...
    <div className="playlist-heading"><span>STRUMENTI</span></div>
    <SidebarMenu><SidebarMenuItem><SidebarMenuButton className="nav-item" onClick={() => { setOpenMobile(false); onDuplicates(); }}><Library /><span>Possibili duplicati</span></SidebarMenuButton></SidebarMenuItem><SidebarMenuItem><SidebarMenuButton className="nav-item" onClick={() => { setOpenMobile(false); onDiagnostics(); }}><SlidersHorizontal /><span>Diagnostica</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu>
...
export default function Home() {
  ...
  const [diagnosticsOpen, setDiagnosticsOpen] = useState(false), [duplicatesOpen, setDuplicatesOpen] = useState(false);
  const [libraryTime, setLibraryTime] = useState(Date.now());
  ...
  const genreOptions = useMemo(() => createOpen ? playlistGenres(tracks) : [], [createOpen, tracks]);
  const newPlaylistTrackIds = useMemo(() => createOpen ? playlistTrackIds(tracks, selectedGenres, addTo?.id) : [], [createOpen, tracks, selectedGenres, addTo?.id]);
  useEffect(() => { if (!createOpen) setSelectedGenres([]); }, [createOpen]);
  ...
  const { currentId, playing, loading: playerLoading, queue, shuffle, repeat, duration, volume } = nativePlayer;
  ...
  async function playerCommand(method: string, params: object = {}) {
    try { await command(method, params); }
    catch (error) { toast.error(error instanceof Error ? error.message : 'Comando audio non riuscito.'); }
  }
  const setVolume = (value: number) => { void playerCommand('volume', { value }); };
  const setRepeat = (value: 0 | 1 | 2) => { void playerCommand('repeat', { value }); };
  useEffect(() => { if (nativePlayer.error) toast.error(nativePlayer.error); }, [nativePlayer.error]);
  ...
  const automatic = AUTO_PLAYLISTS.find(p => p.id === view);
  ...
  const refresh = useCallback(async () => {
    try { const data = await loadLibrary(); setTracks(data.tracks); setPlaylists(data.playlists.sort((a, b) => a.createdAt - b.createdAt)); setLoadError(''); setLibraryTime(Date.now()); }
    catch { setLoadError('La memoria dell’app non è disponibile. Riapri Onda e riprova.'); }
    finally { setReady(true); }
  }, []);
  useEffect(() => { void refresh(); }, [refresh, currentId, playing]);
  useEffect(() => subscribe('libraryChanged', () => { void refresh(); }), [refresh]);
  useEffect(() => subscribe<{ done: number; total: number }>('importProgress', setImportProgress), []);
  ...
  function go(v: View) { setView(v); setSearch(''); setLibraryTime(Date.now()); }
 async function importFiles(files: File[], demo = false) {
  ...
  function beginQueue(track: Track | undefined, ids: string[], random = shuffle) {
    ...
    void playerCommand('setQueue', { ids: unique, ...(track ? { startId: track.id } : {}), shuffle: random });
  }

  async function startTrack(track: Track, list?: string[]) {
    if (queue.includes(track.id)) await playerCommand('select', { id: track.id });
    else beginQueue(track, list || [track.id], shuffle);
  }
  function toggleShuffle() { void playerCommand('shuffle'); }
  function togglePlay() {
    if (!currentId) {
      const ids = queue.length ? queue : selection.rows.length ? selection.rows.map(row => row.track.id) : tracks.map(t => t.id);
      beginQueue(undefined, ids, shuffle || (!queue.length && settings.mode === 'random'));
    } else void playerCommand(playing || playerLoading ? 'pause' : 'play');
  }
  function next() { void playerCommand('next'); }
  function previous() { void playerCommand('previous'); }
  function seek(value: number[]) { void playerCommand('seek', { seconds: value[0] }); }
  function showMediaDiagnostics() { setDiagnosticsOpen(true); }
  async function pickNative(folder: boolean) {
    if (importingRef.current) return;
    importingRef.current = true; setImporting(true); setImportProgress({ done: 0, total: 0 });
    try {
      const result = await request<{ added?: number; duplicates?: number; failed?: number; error?: string }>('pickTracks', { folder }, 30 * 60_000);
      await refresh();
      if (result.added) toast.success(`${result.added} brani aggiunti.`);
      if (result.duplicates) toast.info(`${result.duplicates} brani già presenti.`);
      if (result.failed) toast.error(`${result.failed} file non importati.`, { description: result.error });
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Importazione non riuscita.'); }
    finally { importingRef.current = false; setImporting(false); }
  }
  async function createPlaylist(event: React.FormEvent) {
    event.preventDefault();
    const name = playlistName.trim();
    if (!name || saving || !ready || loadError) return;
    if (selectedGenres.length && !newPlaylistTrackIds.length) {
      toast.error('Nessun brano disponibile per i generi selezionati.');
      return;
    }
    setSaving(true);
    try {
      const p: Playlist = { id: crypto.randomUUID(), name, trackIds: newPlaylistTrackIds, createdAt: Date.now() };
      await savePlaylist(p);
      setPlaylists(old => [...old, p]);
      setPlaylistName(''); setSelectedGenres([]); setCreateOpen(false); setAddTo(null);
      go(p.id);
      toast.success(p.trackIds.length ? `Playlist creata con ${p.trackIds.length} ${p.trackIds.length === 1 ? 'brano' : 'brani'}.` : 'Playlist creata.');
    } catch { toast.error('Non riesco a salvare la playlist. Riprova.'); }
  ...
  async function deletion() {
    ...
    try {
      if (confirm.kind === 'track') { await removeTrack(confirm.id); }
  ...
  const visibleTracks = useMemo(() => automatic ? automaticTracks(tracks, automatic.id, libraryTime) : view === 'favorites' ? tracks.filter(t => t.liked) : playlist ? playlist.trackIds.map(id => tracks.find(t => t.id === id)).filter((t): t is Track => !!t) : view === 'home' ? [...tracks].sort((a, b) => b.addedAt - a.addedAt).slice(0, 5) : tracks, [tracks, view, playlist, automatic, libraryTime]);
  ...
  manualSorting: !!playlist || !!automatic,
  ...
  const title = view === 'search' ? 'Cerca musica.' : view === 'downloads' ? 'Scarica musica.' : view === 'algorithm' ? 'Il tuo algoritmo.' : view === 'home' ? 'La tua musica, oggi.' : view === 'library' ? 'La tua libreria' : view === 'favorites' ? 'I tuoi preferiti' : automatic?.name || playlist?.name || 'La tua libreria';
  ...
  function playbackControls() { return <div className="playback-controls"><IconButton label="Casuale intelligente" active={shuffle} disabled={!tracks.length} onClick={toggleShuffle}><Shuffle size={17} /></IconButton><IconButton label="Brano precedente" disabled={!current} onClick={previous}><SkipBack size={21} fill="currentColor" /></IconButton><IconButton className="main-play" label={
  playerLoading
    ? 'Annulla caricamento'
    : playing
      ? 'Pausa'
...
disabled={!tracks.length} onClick={togglePlay}>{playerLoading ? <Loader2 size={22} className="spin" /> : playing ? <Pause size={23} fill="currentColor" /> : <Play size={23} fill="currentColor" />}</IconButton><IconButton label="Brano successivo" disabled={!current} onClick={() => next()}><SkipForward size={21} fill="currentColor" /></IconButton><IconButton label={repeat === 0 ? 'Attiva ripetizione della coda' : repeat === 1 ? 'Ripeti solo questo brano' : 'Disattiva ripetizione'} active={repeat > 0} disabled={!tracks.length} onClick={() => setRepeat(((repeat + 1) % 3) as 0 | 1 | 2)}>{repeat === 2 ? <Repeat1 size={17} /> : <Repeat size={17} />}</IconButton></div>; }
  ...
  return <SidebarProvider style={{ '--sidebar-width': '232px' } as CSSProperties}><Navigation view={view} go={go} playlists={playlists} counts={[tracks.length, tracks.filter(t => t.liked).length]} onCreate={() => setCreateOpen(true)} bytes={totalBytes} onInfo={() => setInfoOpen(true)} onDuplicates={() => setDuplicatesOpen(true)} onDiagnostics={() => setDiagnosticsOpen(true)} />
  ...
  >
    Diagnostica
      ...
      <div className="page-content"><div className="page-heading"><div><p className="eyebrow">{view === 'search' ? 'TROVA IL TUO PROSSIMO BRANO' : view === 'downloads' ? 'DAL LINK ALLA LIBRERIA' : view === 'algorithm' ? 'SEGUI LE TUE REGOLE' : view === 'home' ? 'BUON ASCOLTO' : automatic ? 'PLAYLIST AUTOMATICA' : playlist ? 'PLAYLIST PERSONALE' : 'LA TUA COLLEZIONE'}</p><h1>{title}</h1><p className="page-subtitle">{view === 'search' ? 'Cerca su Last.fm e aggiungi nuova musica alla tua libreria.' : view === 'downloads' ? 'Scarica un brano o una playlist direttamente sul telefono.' : view === 'algorithm' ? 'Decidi cosa ascoltare dopo. Cambia i pesi e guarda il risultato.' : view === 'home' ? 'La tua libreria. Il tuo ritmo. Le tue regole.' : automatic ? automatic.description : `${visibleTracks.length} ${visibleTracks.length === 1 ? 'brano' : 'brani'}${playlist ? ' · Una selezione tutta tua' : ' · Salvati su questo dispositivo'}`}</p></div>{playlist || automatic ? <div className="heading-actions"><button className="button primary" disabled={!displayed.length} onClick={() => beginQueue(undefined, displayed.map(t => t.id))}><Play size={16} fill="currentColor" />Ascolta</button>{playlist && <IconButton label="Elimina playlist" onClick={() => setConfirm({ kind: 'playlist', id: playlist.id, name: playlist.name })}><Trash2 size={18} /></IconButton>}</div> : view === 'home' && tracks.length > 0 ? <button className="button subtle" onClick={playMix}><Shuffle size={17} />Scegli per me</button> : null}</div>
      ...
      {!ready ? <div className="loading-library" aria-label="Caricamento libreria"><Skeleton className="h-64 w-full" /><Skeleton className="h-16 w-full" /></div> : loadError ? <Empty className="error-state"><EmptyHeader><HardDrive size={36} /><EmptyTitle>Serve l’accesso alla memoria locale</EmptyTitle><EmptyDescription>{loadError}</EmptyDescription></EmptyHeader><button className="button primary" onClick={() => void refresh()}>Riprova</button></Empty> : view === 'search' ? <SearchPanel onDownloads={() => go('downloads')} /> : view === 'downloads' ? <DownloadPanel /> : view === 'algorithm' ? <AlgorithmEditor tracks={tracks} settings={settings} onSave={saveSettings} onPlay={playSelection} onImport={pickFiles} /> : tracks.length === 0 ? <>
        <section className="welcome-grid"><Empty className="import-zone"><div className="import-icon"><FolderOpen size={34} strokeWidth={1.5} /><span><Plus size={13} /></span></div><EmptyHeader><p className="eyebrow">FACCIAMO SPAZIO ALLA TUA MUSICA</p><EmptyTitle className="import-title">La tua collezione<br />parte da qui.</EmptyTitle><EmptyDescription>Scegli i file audio o una cartella dal telefono.</EmptyDescription></EmptyHeader><div className="import-buttons"><button className="button primary" onClick={pickFolder} disabled={importing}><FolderOpen size={17} />Scegli una cartella</button><button className="button outline" onClick={pickFiles} disabled={importing}><Plus size={17} />Aggiungi file</button></div><span className="supported-formats">MP3 <i /> FLAC <i /> WAV <i /> M4A <i /> OGG</span></Empty>
        ...
        {view === 'home' && <><section className="recommendation-section"><div className="section-heading"><div><h2>In sintonia con te <Sparkles size={18} /></h2><p>Scegli la modalità di ascolto o personalizza il tuo algoritmo.</p></div><div className="mix-settings"><Select value={settings.mode} onValueChange={v => void saveSettings({ ...settings, mode: v as ListeningSettings['mode'] })}><SelectTrigger className="discovery-select" aria-label="Modalità di selezione"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="custom">Il mio algoritmo</SelectItem><SelectItem value="random">Casuale intelligente</SelectItem></SelectContent></Select><button className="button outline" onClick={() => go('algorithm')}><SlidersHorizontal size={16} />Personalizza</button></div></div>{selection.error && <p className="formula-error" role="alert">{selection.error} <button className="text-button" onClick={() => go('algorithm')}>Correggi l’algoritmo</button></p>}</section>
        ...
        {view === 'home' && <section aria-label="Playlist automatiche"><h2>Playlist automatiche</h2><div className="auto-playlists">{AUTO_PLAYLISTS.map(p => <button className="auto-playlist-card" key={p.id} onClick={() => go(p.id)}><ListMusic size={20} /><strong>{p.name}</strong><span>Si aggiorna con la tua libreria</span></button>)}</div></section>}
...
/>
    ...
    <DuplicatesDialog open={duplicatesOpen} onOpenChange={setDuplicatesOpen} tracks={tracks} onRefresh={refresh} onPlay={track => beginQueue(track, [track.id], false)} />
